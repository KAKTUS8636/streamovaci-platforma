...
function LikesPage() {
  ...
  const genres = [
    ...new Set(
      items
        .flatMap((entry) =>
          entry.movie.genre ? entry.movie.genre.split(',').map((g) => g.trim()) : []
        )
        .filter(Boolean)
    ),
  ].sort();
  ...
  const filteredItems = items
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        case 'liked_at':
          return dir * (new Date(b.liked_at || 0) - new Date(a.liked_at || 0));
        case 'title':
  ...
  const hasActiveFilters =
    selectedGenres.length > 0 || yearFrom || yearTo || minRating || sortBy !== 'liked_at' || order !== 'desc';
    ...
    <div className="relative">
      ...
      <section className="mb-10 -mx-5 border-y border-white/10 bg-gradient-to-r from-[#070d19] via-[#0b1220] to-[#070d19] px-5 py-10 shadow-[0_24px_90px_rgba(0,0,0,0.42)] lg:-mx-8 lg:px-8 lg:py-14 xl:-mx-12 xl:px-12 xl:py-16">
                ...
                <div className="absolute right-0 top-full z-50 mt-3 w-full min-w-[320px] rounded-xl border border-white/10 bg-[#0b1220] p-6 shadow-2xl shadow-black">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {genres.length === 0 && <p className="text-xs text-gray-600">No genres available</p>}
                    </div>
                  </div>
        ...
        <div className="mt-12 text-center">
          <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.4em] text-red-500/80">Your Favorites</p>
          <h1 className="text-7xl font-black tracking-tighter text-white">Liked Movies</h1>
          <p className="mt-4 text-gray-400">
            {filteredItems.length} {filteredItems.length === 1 ? 'movie' : 'movies'} you loved
          </p>
        </div>
      </section>
      ...
      {!loading && filteredItems.length > 0 && (
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
          {filteredItems.map((entry) => (
            ...
            >
              <Link to={`/movie/${entry.movie_id}`} className="relative aspect-[2/3] w-full overflow-hidden bg-black">
                <img
                ...
                <div className="absolute bottom-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-red-600/80 text-white shadow-lg backdrop-blur-sm">
              ...
              <div className="flex flex-1 flex-col px-4 py-4">
                ...
                {entry.movie.genre && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {entry.movie.genre.split(',').map((g) => g.trim()).filter(Boolean).slice(0, 3).map((g) => (
                      <span key={g} className="rounded-full border border-white/5 bg-white/5 px-2 py-0.5 text-[9px] font-medium text-gray-400">{g}</span>
                    ))}
                  </div>
                )}
                ...
                {entry.liked_at && (
                  <p className="mt-2 text-[10px] text-gray-600">
                    Liked {new Date(entry.liked_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </p>
                )}

                <button
                  onClick={() => unlike(entry.movie_id)}
                  className="mt-3 w-full rounded-lg border border-red-500/10 bg-red-600/10 py-2.5 text-[10px] font-bold uppercase tracking-wider text-red-400 transition-all duration-200 hover:border-red-500/30 hover:bg-red-600/20 hover:text-red-300"
                >
                  ♥ Unlike
                </button>
              </div>
      ...
      {!loading && filteredItems.length === 0 && (
        <div className="rounded-xl border border-white/10 bg-[#0b1220]/60 px-8 py-24 text-center shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
          <p className="mb-4 text-3xl font-bold text-white">
            {items.length === 0 ? 'No liked movies yet' : 'No matches found'}
          </p>
          <p className="mx-auto max-w-xl text-gray-500">
            {items.length === 0
              ? 'Start liking movies to build your favorites collection.'
